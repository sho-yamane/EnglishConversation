...
class SharedViewModel {
    ...
    func sendMessage(message: String, sender: MessageModel.Sender) {
        let allMessages = messageDataSets.compactMap { message -> ModelContent? in
            do {
                return try ModelContent(role: message.role, message.message)
            } catch {
                print("Error creating ModelContent: \(error)")
                return nil
            }
        }
